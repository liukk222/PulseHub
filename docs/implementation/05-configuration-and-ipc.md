...
> **Agent 导读：** 本文支撑根目录 [`AGENTS.md`](../../AGENTS.md) 的重构与新功能开发。修改 schema、原子存储或 IPC 时，必须保持 agent 唯一写入、当前登录会话隔离、严格输入校验及“保存不等于应用”。
...
运行状态，未修改其自动启动设置。设备仍处于 `Host` 模式；板载内容持久化成功不等同于当前正在使用
板载配置。当前 G102 只有一个用户板载配置，完整环境切换会按需同步板载配置；自动切换通过 1 秒稳定窗口、5 秒冷却和整扇区幂等比较限制闪存写入。
...
| `commit_config`      | 带 `base_revision` 原子保存；不自动应用设备配置           |
| `apply_now`          | 显式串行应用当前环境的完整设备配置                        |
| `shutdown`           | 安全还原鼠标后退出代理；`force=true` 明确跳过恢复并退出   |
| `set_selection_mode` | 切换自动/手动环境                                         |
| `attach_ui`          | DTO 已定义；当前 agent 未为其实现专用处理，按默认快照响应 |

`shutdown` 由当前登录会话内受 Named Pipe DACL 保护的已协商客户端调用。`force=false` 时，设备协调线程进入停止状态，阻断后续前台环境应用，应用用户配置的 `shutdown_profile` 并回读完整设备配置；首次生成配置时该 profile 默认为 `1600 DPI`、`1000 Hz`、默认四档 DPI 和原生鼠标按键。可重试 HID 超时最多尝试 3 次、总预算 10 秒。设备离线时在预算耗尽后记录审计日志并退出；设备在线且恢复失败时返回可重试错误，由 GUI 提供“重试恢复 / 仍然退出”。`force=true` 不声称设备已恢复，只记录强制退出审计事件。

`Event` DTO 中定义了 `snapshot_changed`、`apply_started`、`apply_finished`、`device_changed` 与 `activate_ui`，但当前 Named Pipe 会话为请求/响应模式，agent 尚未主动推送这些事件。
