...
- 设备恢复后重新应用当前应生效的配置；当前恢复依赖定期 DPI 健康回采，尚未接入 Windows 睡眠恢复通知。
...
这条进程边界保证代理是设备和持久化状态的唯一写入者。关闭主窗口会结束 GUI 事件循环，agent 托盘和代理继续运行；重新打开时由 agent 启动新的 GUI 进程。选择“退出托盘”则请求代理应用用户配置的 `shutdown_profile` 并结束整个 PulseHub；首次生成配置时其默认值为 `1600 DPI`、`1000 Hz`、默认四档 DPI 和原生鼠标按键。
