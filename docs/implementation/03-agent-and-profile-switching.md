...
| 线程/路径          | 当前职责                                                        | 阻塞点或等待方式             |
| ------------------ | --------------------------------------------------------------- | ---------------------------- |
| 前台/命令协调路径  | `watcher::run`、前台事件合并、命令队列与串行 HID 协调           | 最长 `250 ms` 的通道接收超时 |
| 前台 hook 专用线程 | `win_event_hook` 的 `SystemForeground` 回调，仅 `try_send` 通知 | hook 库管理的消息循环        |
| Slint 托盘线程     | agent 的 `AppTray` 托盘交互                                     | Slint 事件循环               |
| IPC 监听/会话线程  | Named Pipe 监听；每个活动客户端的请求/响应会话                  | 管道 I/O                     |

当前没有独立的 Win32 隐藏窗口、设备/电源通知消息线程或 `GetMessageW` 托盘宿主；不得为了追求线程数字在事件回调中执行阻塞操作。
...
2. 若设备在线，应用用户配置的 `shutdown_profile`（首次生成配置时默认为 `1600 DPI`、`1000 Hz`、默认四档 DPI 和原生鼠标按键）。
3. 回读完整设备配置；板载写入仍使用幂等、防抖和事务保护。
4. 恢复成功后停止接受新 IPC 请求，并注销 WinEvent hook。
5. 请求设备线程关闭 HID 句柄并等待线程结束。
6. 完成日志刷新，销毁消息窗口并释放单实例互斥体。
7. 销毁 agent 自身的托盘图标并退出；由 agent 启动的 GUI 进程也随安全退出流程结束。
