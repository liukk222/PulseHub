...
> **Agent 导读：** 本文为根目录 [`AGENTS.md`](../../AGENTS.md) 的三类允许开发方向提供验证、发布和风险控制依据。普通自动化测试不得写入真实硬件；新增物理设备必须补充独立 POC 与实机验收。
...
- 当前 agent 依赖 Slint 承载托盘，且存在 IPC/托盘/hook 线程；本条是未来重构目标，不能作为当前发布验收结论。
- 当前存在 1 秒托盘刷新和 2 秒 DPI 健康回采；性能验收需先量化其成本，再决定是否重构为其他模型。
...
- 验证关闭 GUI 后编辑器进程退出、agent 托盘继续运行，并能再次启动 GUI。
