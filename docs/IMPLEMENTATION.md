...
映射编码进同一个 G102 板载扇区事务，执行整扇区 CRC 与写后回读，最后启用板载模式。启用板载模式
后禁止再次执行运行态 DPI 写入，否则设备会回到 `Host` 模式并停用板载按键。目标扇区已一致时跳过
闪存写入；IPC 为该显式事务保留 15 秒响应窗口。该操作不提交配置，但会显式写入板载闪存。
...
这修复了旧实现只调用 `set_first_g102_dpi`、从未把 GUI 映射发送到设备的问题。前台环境自动切换仍
只改运行态 DPI，不在窗口切换时高频擦写闪存；需要切换整套按键时由用户明确点击“重新应用”。
