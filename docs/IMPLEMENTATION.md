...
当前工作区已初始化 Cargo Workspace，并包含领域模型、设备接口、配置切换、配置存储、IPC 与三个可执行入口的基础骨架；Win32 托盘、Slint GUI、Named Pipe 传输和 Logitech HID++ 通信仍未实现。因此，本文描述的是后续编码应遵循的实现基线，而不是对完整现有功能的说明。文中的性能数字均为待验证目标；HID++ 协议细节均需通过真实设备验证后才能转为“已实现”结论。
...
| 项目 | 当前结论 | 实现要求 |
|---|---|---|
| DPI 范围 | 官方页面标称 `200–8,000` | 仍从设备返回值构建可选集合 |
| 可编程按键 | 官方页面标称 6 个 | 逐个查询控制 ID 和允许动作 |
| 常见 VID/PID | `046d:c092` 等 | 仅用于候选过滤，不作为最终能力判断 |
| `ADJUSTABLE_DPI 0x2201` | 开源 HID++ 实现使用 | 待目标固件查询确认 |
