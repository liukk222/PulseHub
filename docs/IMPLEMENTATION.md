...
当前工作区已初始化 Cargo Workspace，并包含领域模型、设备接口、配置切换、配置存储、IPC 与三个可执行入口的基础骨架；阶段 0 的 Windows HID 枚举、HID++ 只读功能发现和 DPI 能力读取已经通过 G102 实机验证。Win32 托盘、Slint GUI、Named Pipe 传输以及 DPI/按键写入仍未实现。因此，本文同时记录已验证实现与后续编码基线；未明确标为实机验证的性能数字和协议细节仍属于待验证目标。
...
| `ADJUSTABLE_DPI 0x2201` | 实机确认 index `0x0A`、version `1` | 已实现只读范围与当前值查询，写入待 POC |
| `REPROG_CONTROLS_V4 0x1B04` | 实机功能表未发现 | 后续从板载配置能力验证可用映射路径 |
| `ONBOARD_PROFILES 0x8100` | 实机确认 index `0x0F`、version `0`、1 个用户配置和 6 个按键 | 待确认扇区内容、写入语义和持久性 |
...
并能读取当前 DPI。`ONBOARD_PROFILES 0x8100` 存在，但尚未查询其配置内容；功能表中未发现
